...
class ConfigurationFactory(AbstractBoardFactory):
    def __init__(self):
        super().__init__()
        self.mapping['hwconf'] = self.tuplelist
        self.mapping['conditions'] = self.tuplelist
    ...
    def loadConfiguration(self,inifile):
        ...
        try:
            ...
            temp = reader['main'].get('deltatime',1)
            temp = self.process(temp)
            conf.put('deltatime', temp)
            #todo: refactoring cancellare le due righe seguenti
